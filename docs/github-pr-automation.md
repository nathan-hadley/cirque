...
2. ✅ **NEW:** GitHub PR is automatically created with:
...
3. ✅ **NEW:** GitHub Action automatically runs `pnpm sync-data` and commits TypeScript files
...
API validates submission
    ↓
GitHub PR created (CRITICAL PATH)
...
GitHub Action triggered
    ├─ Runs on any PR with GeoJSON changes
    ├─ Runs pnpm sync-data
...
- **PR creation is the critical path:**
  - If PR creation fails → Request fails with error
  - If PR succeeds → Request succeeds ✅
  
- **Email is optional notification:**
  - If email fails after PR succeeds → Request still succeeds, error is logged
  - If email succeeds → Bonus notification sent
  
- **Why this order?**
  - PR actually adds the problem to the app (critical)
  - Email is just a notification/backup (nice-to-have)
  - Failed email can be checked in logs, but problem is safely in the system
